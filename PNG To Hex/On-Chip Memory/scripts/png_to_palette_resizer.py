...
palette_filename = "./palette.txt" #input("What's the palette file path?")
palette_hex = []
plt_file = open(palette_filename,'r')
line = plt_file.readline()
while line:
    spt = line.strip().split(':')
    line = plt_file.readline()

    if len(spt)>1: plt_hex = "0x"+spt[1].strip()
    else: continue

    palette_hex.append(plt_hex)
